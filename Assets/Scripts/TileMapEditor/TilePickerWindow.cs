    ...
    void OnGUI()
    {
        ...
        if(selection != null)
        {
            ...
            if(texture2D != null)
            {
                ...
                boxTex.SetPixel(0, 0, new Color(0, 0.4f, 1f, 0.4f));
                boxTex.Apply();
                ...
                if(cEvent.type == EventType.mouseDown && cEvent.button == 0)
                {
                    ...
                    if (currentSelection.x > grid.x - 1) { currentSelection.x = grid.x - 1; }
                    if (currentSelection.y > grid.y - 1) { currentSelection.y = grid.y - 1; }
