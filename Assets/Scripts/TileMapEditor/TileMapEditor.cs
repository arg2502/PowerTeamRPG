...
public class TileMapEditor : Editor {
    ...
    bool mouseOnMap { get { return mouseHitPos.x > 0 && mouseHitPos.x < map.gridSize.x && mouseHitPos.y < 0 && mouseHitPos.y < map.gridSize.y; } }
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox("You have not selected a Texture2D yet", MessageType.Warning);
        }
        else
        {
    ...
    void UpdateCalculations()
    {
        ...
        var sprite = (Sprite)map.spriteReferences[1]; // second item, because 1st is the whole texture2D, not the spliced sprites
    ...
    {
        var p = new Plane(map.transform.TransformDirection(Vector3.forward), Vector3.forward);
