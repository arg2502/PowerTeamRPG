...
public class ShopkeeperBuyMenu : Menu
{
    ...
    protected override void AddButtons()
    {
        ...
        for (int i = 0; i < CurrentShopkeeper.shopInventory.Count; i++)
        {
            ...
            item.name = CurrentShopkeeper.shopInventory[i].name;
    ...
    {
        skSpeaker.text = gameControl.Character.CurrentShopkeeper.npcName;
        dictComments = new Dictionary<string, ShopkeeperComment>();

        // 0 - item name, 1 - emotion, 2 - comment
        if (CurrentShopkeeper.commentList == null) return;
        var rows = CurrentShopkeeper.commentList.text.Split('\n');
        ...
        for(int i = 0; i < rows.Length; i++)
        {
            ...
            if (string.IsNullOrEmpty(emotion))
                _sprite = CurrentShopkeeper.neutralSpr;
            else
            {
                if (string.Equals(emotion, "HAPPY"))
                    _sprite = CurrentShopkeeper.happySpr;

                else if (string.Equals(emotion, "SAD"))
                    _sprite = CurrentShopkeeper.sadSpr;

                else if (string.Equals(emotion, "ANGRY"))
                    _sprite = CurrentShopkeeper.angrySpr;
            }
    ...
    void UpdateComment(ScriptableItem newItem)
    {
        ...
        {
            skPortrait.sprite = CurrentShopkeeper.neutralSpr;
